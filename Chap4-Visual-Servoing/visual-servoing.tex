...
\begin{equation*}
 ...
 \left(
 \begin{array}{c}
 \gamma U^{\transpose}_{z}(S_{z} \hat{x}_k - V_c X_k^{fc}) \\
 -\gamma V^{\transpose}(S_{z}\hat{x}_k - V_c X_k^{fc}) \\
 \gamma U^{\transpose}_{z}(S_{z} \hat{y}_k - V_c Y_k^{fc}) \\
 -\gamma V^{\transpose} (S_{z}\hat{y}_k - V_c Y_k^{fc})
 \end{array}
 \right),
\end{equation*}
...
\begin{equation*}
 ...
 \left(
 \begin{matrix}
 \beta  \sum\limits_l U_{u}^{\transpose} A_{l,k}^{\transpose} W [A_{l,k} U_{s} \hat{x}_k + B_{l,k} U_{s} \hat{y}_k + C_{l,k} - S^d_l]\\
 0 \\
 \beta  \sum\limits_l U_{p}^{\transpose} B_{l,k}^{\transpose} W [A_{l,k} U_{s} \hat{x}_k + B_{l,k} U_{s} \hat{y}_k + C_{l,k} - S^d_l ]\\
 0
 \end{matrix}
 \right).
\end{equation*}
...
\begin{eqnarray}
\label{Eq:RotConstFT}
| \Theta_{k+1} - \Theta^f_{k+1}| < \Theta^{FT}_{max} \\
\label{Eq:RotConstLR}
| \Theta^{f,L}_{k+1} - \Theta^{f,R}_{k+1}| < \Theta^{LR}_{max} \\
\label{Eq:RotConstVis}
| \Theta_{k+1}| < \Theta^{visibility}.
\end{eqnarray}
...
\begin{figure*}[ht]
 ...
 \caption{\label{Fig:Comparisons-approaches} Comparison between the coupled (upper part) and decoupled approaches (lower part). On the left side, we depict the robot trajectory with the footsteps (red and blue), and the CoM (black) and CoP (pink) trajectories. On the right side, we depict the evolution of the instantaneus features positions during the experiment (from the red rectangle, at the beginning of the simulation, to the blue one, at the end).}
 \end{figure*}
